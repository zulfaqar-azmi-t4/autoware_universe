  ...
  const std::shared_ptr<const PlannerData> planner_data)
{
  ...
  processing_time_publisher_->publish(std::invoke([&]() {
    ...
    fmt::print("Processing time for {}: {:.3f} ms\n", get_module_name(), msg.data);
    return msg;
  }));
  ...
  const std::shared_ptr<const PlannerData> & planner_data)
{
  ...
  for (const auto side_key : g_side_keys) {
    fmt::print(
      "closest size {}, {}\n", magic_enum::enum_name(side_key),
      output_.closest_projections_to_bound[side_key].size());
  }
  ...
  output_.is_critical_departing = std::any_of(
    ...
    [&th_trigger, &planner_data, &ego_dist_on_traj_with_offset_m](const DeparturePoint & pt) {
      const auto braking_start_vel = std::clamp(
        planner_data->current_odometry.twist.twist.linear.x, th_trigger.th_vel_mps.min,
        th_trigger.th_vel_mps.max);
      ...
      fmt::print(
        "Diff {} braking_dist {} [m]\n",
        pt.dist_on_traj - ego_dist_on_traj_with_offset_m(planner_data->is_driving_forward),
        braking_dist);
  ...
  toc_curr_watch("published_marker");

  // for(const auto & [key, time]: processing_times_ms_){
  //   fmt::print("{}. {}\n",key, time);
  // }

  ...
  const std::vector<Segment2d> & segments1, const std::vector<Segment2d> & segments2)
{
  ...
  for (const auto & [p1, p2] : segments1) {
    ...
    rtree_bbox.query(
      bgi::nearest(p1, num_of_boundary_segment_to_query),
...
Output LaneDepartureChecker::update(const Input & input)
{
  ...
  if (!output.ego_footprint_side.left.empty()) {
    const auto back_point = universe_utils::toMsg(
  ...
  const auto dist_ego_left_to_boundary = predicted_path_to_lane_boundary_dist(
    output.ego_footprint_side.left, output.lane_boundary.left);
  ...
  if(dist_ego_left_to_boundary && dist_ego_right_to_boundary){
    const auto min_to_print = std::min(dist_ego_left_to_boundary->size(),
    dist_ego_right_to_boundary->size());
    ...
    for(size_t i = 0; i < min_to_print; ++i){
      ...
      fmt::print("at idx {} left = {} [m], right = {} [m]\n", i,
      dist_ego_left_to_boundary->at(i).second, dist_ego_right_to_boundary->at(i).second);
    }
  }
